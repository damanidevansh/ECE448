...
def training(sentences):
    ...
    hapaxtag_count = defaultdict(lambda:0)
    for words in word_count:
        if word_count[words] == 1:
            for tag in eprob_count:
                if words in eprob_count[tag]:
                    hapaxtag_count[tag]+=1
    
    hapaxprob = defaultdict(lambda:0)
    total_hapax = sum(hapaxtag_count.values())
    for tag in hapaxtag_count:
        hapaxprob[tag] = hapaxtag_count[tag] / total_hapax
    
    for tag in eprob_count.keys():
        total = sum(eprob_count[tag].values())
        length = len(eprob_count[tag])
        if hapaxprob[tag] == 0:
            weight = epsilon_for_pt
        else:
            weight = hapaxprob[tag]
        denominator = ((total+1)*emit_epsilon*weight) + length
        ...
        emit_prob[tag]['UNKNOWN'] = (emit_epsilon * weight) / denominator
    ...
    for tag in tagset:
        ...
        for i in prev_prob:
            ...
            if tag in emit_prob and etag in emit_prob[tag]:
                if (emit_prob[tag][etag] > 0):
                    emits = log(emit_prob[tag][etag])
                else:
                    emits = log(emit_epsilon) 
            else:
