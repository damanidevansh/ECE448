...
def best_first_search(starting_state):
    ...
    while frontier:
        ...
        for neighbor in curr_state.get_neighbors():
            node_distance = neighbor.dist_from_start + neighbor.h
            
            if (neighbor not in visited_states) or ((node_distance + neighbor.compute_heuristic()) < visited_states[neighbor][dist]):
                visited_states[neighbor] = (curr_state, node_distance + neighbor.compute_heuristic())
